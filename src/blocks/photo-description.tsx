...
const DESCRIBE_PRESET_IDS = [
  'describe-detail',
  'describe-briefly',
  'caption-title',
] as const;

const HOW_STEP_KEYS = ['upload', 'detail', 'copy'] as const;

export const PHOTO_DESCRIPTION_FAQ_KEYS = [
  'what',
  'free',
  'detail',
  'languages',
...
export function PhotoDescriptionHero() {
  return (
...
export function PhotoDescriptionTool() {
  return (
    <ImageDescriberTool
      title={m['photoDescription.tool.title']()}
      description={m['photoDescription.tool.description']()}
      tip={m['photoDescription.tool.tip']()}
      presetIds={DESCRIBE_PRESET_IDS}
      defaultPresetId="describe-detail"
    />
  );
}

export function PhotoDescriptionHowItWorks() {
  return (
    <section className="px-4 py-24 sm:py-32">
      <div className="mx-auto max-w-5xl">
        <div className="text-center mb-16">
          <h2 className="font-serif font-medium text-4xl sm:text-5xl tracking-tight">
            {m['photoDescription.how.title']()}
          </h2>
          <p className="mt-5 text-muted-foreground">
            {m['photoDescription.how.description']()}
          </p>
        </div>
          ...
          {HOW_STEP_KEYS.map((key, index) => (
            <div key={key} className="rounded-3xl border border-dashed p-8">
              ...
              <h3 className="mt-4 text-lg font-semibold">
                {tDynamic(`photoDescription.how.steps.${key}.title`)}
              </h3>
              <p className="mt-2 text-muted-foreground leading-relaxed">
                {tDynamic(`photoDescription.how.steps.${key}.text`)}
              </p>
            </div>
          ))}
        </div>
